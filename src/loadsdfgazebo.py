...
'''
Fix for sdf_spawner buggy race conditions
'''
#!/usr/bin/env python
#
...
## 2018 , fixed for initial configuration of complex robot with multiple joints for sdf -- Usama Siraj
...
class SpawnModel():
    ...
    def callSpawnService(self):
        ...
        old_physics_status = self.is_physics_enabled()
        rospy.loginfo('Status of Physics is %d', old_physics_status)
        if old_physics_status:
            rospy.loginfo('Temporarily Pausing Physics')
        self.pause_physics()
        ...
        if len(self.joint_names) != 0:
          #time.sleep(5) # wait before call for model configuration
          try:
        ...
        # unpause physics if it was stopped
        if old_physics_status:
    ...
    def pause_physics(self):
        rospy.wait_for_service('%s/pause_physics' % (self.gazebo_namespace))
        try:
            npause_physics = rospy.ServiceProxy('%s/pause_physics' % (self.gazebo_namespace), Empty)
            npause_physics()
        except rospy.ServiceException, e:
    ...
    def is_physics_enabled(self):
        rospy.wait_for_service('%s/get_physics_properties' % (self.gazebo_namespace))
        try:
            npause_physics = rospy.ServiceProxy('%s/get_physics_properties' % (self.gazebo_namespace), GetPhysicsProperties)
            status = npause_physics()
            return not status.pause
        except rospy.ServiceException, e:
            rospy.logerr("Pause physics service call failed: %s", e)
